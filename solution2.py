...
for island, coords in islands.items():
    ...
    all_data[island] = SST_clim1.variables['analysed_sst'][:, i1[0], i2[0]].squeeze() - 273.15
...
for island, coords in islands2.items():
    ...
    # Replace this line with your data extraction logic
    all_data[island] = SST_clim2.variables['analysed_sst'][:, j1[0], j2[0]].squeeze() - 273.15

# Create a DataFrame for all island data
all_data_df = pd.DataFrame(all_data)

# Save the data to a single CSV file
all_data_df.to_csv('2021_data.csv.csv', index=False)
